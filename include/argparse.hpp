...
namespace argparse {
    ...
    public:
        ...
        // Constructor to signal if this is the main command or a subcommand
        Args(bool is_main = false): is_main(is_main) {};
        ...
        virtual void help() {
            ...
            if (!subcommand_entries.empty()) {
                ...
                for (const auto &[subcommand, subentry]: subcommand_entries) {
                    cout << subcommand;
                    if (i++ < subcommand_entries.size() - 1)
                        cout << " | ";
                }
            ...
            for (const auto &entry : arg_entries) {
                cout << setw(HELP_MAX_ARG_CHARS) << entry->keys_[0] << " : " << entry->help << entry->info() << endl;
            }

            cout << "Options:" << endl;
            for (const auto &entry : all_entries) {
                if (entry->type != Entry::ARG) {
                    cout << setw(HELP_MAX_ARG_CHARS) << entry->_get_keys() << " : " << entry->help << entry->info() << endl;
                }
            }
